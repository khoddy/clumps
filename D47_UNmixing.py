...
# calc isotope ratios for sample data
R13_sample = (data[:,0]/1000 +1) * 0.0112372
R18_sample = (data[:,1]/1000 +1) * 0.0020052
R17_sample = (R18_sample/0.0020052)**0.5164 * 0.0003799
C12_sample = 1/(1+R13_sample)
C13_sample = R13_sample/(1+R13_sample)
O16_sample = 1/(1+R17_sample+R18_sample)
O17_sample = R17_sample/(1+R17_sample+R18_sample)
O18_sample = R18_sample/(1+R17_sample+R18_sample)
R45_sample_stoc = (C12_sample*O16_sample*O16_sample + 2*C12_sample*O16_sample*O17_sample)/(C12_sample*O16_sample*O16_sample)
R46_sample_stoc = (2*C12_sample*O16_sample*O18_sample + C12_sample*O17_sample*O17_sample + 2*C13_sample*O16_sample*O17_sample)/(C12_sample*O16_sample*O16_sample)
R47_sample_stoc = (2*C13_sample*O16_sample*O18_sample + C13_sample*O17_sample*O17_sample + 2*C12_sample*O17_sample*O18_sample)/(C12_sample*O16_sample*O16_sample)
d45_sample = (R45_sample_stoc/R45_rg_stoc - 1)*1000
d46_sample = (R46_sample_stoc/R46_rg_stoc - 1)*1000
D47_sample_RF = data[:,2] - ac
D47_sample_vsWG = (D47_sample_RF - ETF_int)/ETF_slope
d47_sample = ((D47_sample_vsWG + 1000)*R47_sample_stoc - 1000*R47_rg_stoc)/(R47_rg_stoc - EGL_slope*R47_sample_stoc)



### Unmixing mixed samples using d13C Mixing ratios (assigned endmember d13C values)

# Initialize some arrays:
d18O_e2_calc = np.array([])
d13C_e2_calc = np.array([])
d45_e2_calc = np.array([])
d46_e2_calc = np.array([])
d47_e2_calc = np.array([])

# Do the calculations, looping through to select "mixed" samples...
for i in range(len(data[:,8])):
	if data[i,8] == 2:
		d13C_e2_calc = np.append(d13C_e2_calc,((data[i,0]-d13C_mixRatios[i]*d13C_e1)/(1-d13C_mixRatios[i])))
		d18O_e2_calc = np.append(d18O_e2_calc,((data[i,1]-d13C_mixRatios[i]*d18O_e1)/(1-d13C_mixRatios[i])))
		d45_e2_calc = np.append(d45_e2_calc,((d45_sample[i]-d13C_mixRatios[i]*d45_e1)/(1-d13C_mixRatios[i])))
		d46_e2_calc = np.append(d46_e2_calc,((d46_sample[i]-d13C_mixRatios[i]*d46_e1)/(1-d13C_mixRatios[i])))
		d47_e2_calc = np.append(d47_e2_calc,((d47_sample[i]-d13C_mixRatios[i]*d47_e1)/(1-d13C_mixRatios[i])))
	
R13_e2_calc = (d13C_e2_calc/1000 + 1) * 0.0112372
R18_e2_calc = (d18O_e2_calc/1000 + 1) * 0.0020052
R17_e2_calc = (R18_e2_calc/0.0020052)**0.5164 * 0.0003799
C12_e2_calc = 1/(1+R13_e2_calc)
C13_e2_calc = R13_e2_calc/(1+R13_e2_calc)
O16_e2_calc = 1/(1+R17_e2_calc+R18_e2_calc)
O17_e2_calc = R17_e2_calc/(1+R17_e2_calc+R18_e2_calc)
O18_e2_calc = R18_e2_calc/(1+R17_e2_calc+R18_e2_calc)
R45_e2_calc = (d45_e2_calc/1000 + 1)*R45_rg_stoc
R46_e2_calc = (d46_e2_calc/1000 + 1)*R46_rg_stoc
R47_e2_calc = (d47_e2_calc/1000 + 1)*R47_rg_stoc
R47_e2_calc_stoc = ((2*C13_e2_calc*O16_e2_calc*O18_e2_calc + C13_e2_calc*O17_e2_calc*O17_e2_calc + 2*C12_e2_calc*O17_e2_calc*O18_e2_calc)/(C12_e2_calc*O16_e2_calc*O16_e2_calc))
R45_e2_calc_stoc = (C12_e2_calc*O16_e2_calc*O16_e2_calc + 2*C12_e2_calc*O16_e2_calc*O17_e2_calc)/(C12_e2_calc*O16_e2_calc*O16_e2_calc)
R46_e2_calc_stoc = (2*C12_e2_calc*O16_e2_calc*O18_e2_calc + C12_e2_calc*O17_e2_calc*O17_e2_calc + 2*C13_e2_calc*O16_e2_calc*O17_e2_calc)/(C12_e2_calc*O16_e2_calc*O16_e2_calc)
D47_e2_calc_vsWG = ((R47_e2_calc/R47_e2_calc_stoc - 1) - (R46_e2_calc/R46_e2_calc_stoc - 1) - (R45_e2_calc/R45_e2_calc_stoc -1))*1000
D47_e2_calc_vsWGo = D47_e2_calc_vsWG - d47_e2_calc*EGL_slope
D47_e2_calc = D47_e2_calc_vsWGo*ETF_slope + ETF_int
...
# Plot up mixing model and sample data (4 panel subplot)
plt.figure(0)
plt.subplot(2,2,1)
...
# New figure for "unmixed" calculation data
plt.figure(1)
...
plt.plot(d13C_e1,d18O_e1,'ro')
plt.plot(d13C_e2_calc,d18O_e2_calc,'rd')
plt.xlabel("d13C VPDB")
plt.ylabel("d18O VSMOW")

plt.subplot(2,2,2)
plt.title("d45 vs d47")
for i in range(len(data[:,8])):
	if data[i,8] == 1:
		plt.plot(d45_sample[i],d47_sample[i], 'k.')
	elif data[i,8] == 2:
		plt.plot(d45_sample[i],d47_sample[i], 'b.')
	else:
		plt.plot(d45_sample[i],d47_sample[i], 'g.')

plt.plot(d45_e1,d47_e1,'ro')
plt.plot(d45_e2_calc,d47_e2_calc,'rd')
plt.xlabel("d45")
plt.ylabel("d47")
...
plt.plot(d13C_e1,D47_e1,'ro')
plt.plot(d13C_e2_calc,D47_e2_calc,'rd')
plt.xlabel("d13C VPDB")
plt.ylabel("D47")
...
plt.plot(d18O_e1,D47_e1,'ro')
plt.plot(d18O_e2_calc,D47_e2_calc,'rd')
plt.xlabel("d18O VSMOW")
plt.ylabel("D47")
